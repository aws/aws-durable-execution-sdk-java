...
class InvokeIntegrationTest {
    ...
    void testInvokeWithSuccessResult() {
        ...
        runner.completeChainedInvoke(
                "invoke",
                new LocalDurableTestRunner.ChainedInvokeResult(OperationStatus.SUCCEEDED, "\"test output\"", null));
    ...
    void testMultipleInvokeAsyncWithSuccessResult() {
        ...
        runner.completeChainedInvoke(
                "invoke1",
                new LocalDurableTestRunner.ChainedInvokeResult(OperationStatus.SUCCEEDED, "\"test output 1\"", null));
        var output2 = runner.run(new TestInput("test"));
        ...
        runner.completeChainedInvoke(
                "invoke2",
                new LocalDurableTestRunner.ChainedInvokeResult(OperationStatus.SUCCEEDED, "\"test output 2\"", null));
    ...
    void testInvokeWithFailedResults() {
        ...
        runner.completeChainedInvoke(
                "invoke",
                new LocalDurableTestRunner.ChainedInvokeResult(
        ...
        assertEquals("InvokeFailedException", error.errorType());
