 ...
 *
 * <p>pre(step-&gt;1) -&gt; sub(nested dag[pre]: n1-&gt;2, n2[n1]=n1+3=5) -&gt; post(step[sub]=sub.n2*10=50). maxConcurrency=1
 * at both DAG levels for a deterministic topological order. Every task succeeds → ALL_COMPLETED at both levels. Returns
 * the canonical summary from 10-9.yaml.
 */
        ...
        DagResult r = context.dag(
                ...
                d -> {
                    ...
                    d.step("post", Integer.class, (deps, s) -> {
                                ...
                                return ((Number)
                                                nested.getResult("n2").orElseThrow())
                                                .intValue()
                                        * 10;
                            })
                            .reads(sub);
