 ...
 * swap=false makes {@code a} finish first; swap=true makes {@code b} finish first. Both invocations register the
 * SAME task names in the SAME order every time &mdash; only the RUNTIME completion order changes.
 *
 ...
 * single-invocation validator) with swap flipped between runs, and asserts each task's {@code Id} field in the
 * captured execution history is IDENTICAL across both runs &mdash; the direct proof that ids are derived from the
 * task name, not from completion order or a counter.
 */
public class DagIdStability extends DurableHandler<Map<String, Object>, Map<String, Object>> {
    ...
    public Map<String, Object> handleRequest(Map<String, Object> input, DurableContext context) {
        ...
        DagResult r = context.dag("idstabilitydag", d -> {
            ...
            var b = d.step("b", String.class, (deps, s) -> {
                    ...
                    .after(root);
            var afterA = d.step(
                            "afterA", String.class, (deps, s) -> deps.get(a).orElseThrow() + "a")
                    .reads(a);
            var afterB = d.step(
                            "afterB", String.class, (deps, s) -> deps.get(b).orElseThrow() + "b")
                    .reads(b);
            d.step(
                            "merge",
                            String.class,
                            (deps, s) -> deps.get(afterA).orElseThrow() + deps.get(afterB).orElseThrow())
                    .reads(afterA, afterB);
        });
