...
def read_examples_jar_path() -> str:
    ...
    if artifact_id is None or version is None:
        raise ValueError("Unable to read examples artifactId/version from pom.xml")
    return f"/var/task/lib/{artifact_id}-{version}.jar"
...
def render_template(examples: list[ExampleFunction]) -> str:
    java_agent_extension_path = read_examples_jar_path()
    lines = [
