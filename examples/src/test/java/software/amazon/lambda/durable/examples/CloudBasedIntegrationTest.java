...
class CloudBasedIntegrationTest {
    ...
    void testFileSystemSerDesExample() throws Exception {
        ...
        assertPreview(storedEnvelope, "RESULT", "store-payload", input.id(), value.length(), null);

        var verifiedEnvelope = assertFileSystemEnvelope(
                result.getOperation("verify-payload").getStepDetails().result());
        assertPreview(verifiedEnvelope, "RESULT", "verify-payload", input.id(), value.length(), expectedChecksum);
        ...
        assertPreview(outputEnvelope, "OUTPUT", null, input.id(), value.length(), expectedChecksum);
    }
    ...
    private static void assertPreview(
            JsonNode envelope, String payloadKind, String operationName, String id, int length, String checksum) {
        var preview = envelope.get("preview");
        assertEquals(payloadKind, preview.get("payloadKind").textValue());
        if (operationName != null) {
            assertEquals(operationName, preview.get("operationName").textValue());
        }
